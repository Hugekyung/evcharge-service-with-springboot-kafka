...
애플리케이션이 시작되면 Kafka Admin 설정을 통해 `charging-events` 토픽이 자동으로 생성된다. Kafka가 아직 준비되지 않은 경우 1초, 2초, 4초, 8초 간격으로 총 5회 시도(최초 1회 + 재시도 4회)하며, 연결 timeout이나 일시적인 broker 오류만 재시도한다. 인증·권한·토픽 설정 오류처럼 재시도로 해결되지 않는 오류는 즉시 애플리케이션 시작을 실패시킨다. 일시적인 오류도 끝까지 해결되지 않으면 애플리케이션 시작을 실패시킨다. 토픽 생성이 성공한 뒤 Kafka Listener를 시작한다.
